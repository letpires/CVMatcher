...
def get_github_projects(state: AgentState) -> AgentState:
...
def summarize_linkedin_data(state: AgentState) -> AgentState:
...
def compare_resume_with_job(state: AgentState) -> AgentState:
...
def generate_tailored_resume(state: AgentState) -> AgentState:
...
def push_to_ipfs(state: AgentState) -> AgentState:
...
def save_version_history(state: AgentState) -> AgentState:
...
# Define the workflow
def create_resume_analyzer_workflow() -> CompiledGraph:
    # Create the graph
...
# Create the workflow
resume_analyzer_workflow = create_resume_analyzer_workflow()
...
def analyze_resume_and_job(resume: str, job_description: str, github_username: str = None, linkedin_url: str = None) -> Dict:
    ...
    # Run the workflow
    result = resume_analyzer_workflow.invoke(state)
